...
- `apply_patch(diff)` — preferred over whole-file writes for edits
- `run_engine(command)` — via the EngineAdapter, never a raw shell to `godot`
...
**Interface (every engine implements this):**
```python
class EngineAdapter(Protocol):
    def create_project(self, path: str, name: str) -> None: ...
    def write_scene(self, scene: SceneSpec) -> None: ...
    def write_script(self, path: str, code: str) -> None: ...
    def add_asset(self, src: str, dest: str) -> None: ...
    def run(self, headless: bool = True, timeout_s: int = 30) -> RunResult: ...   # returns logs
    def screenshot(self, out_path: str) -> None: ...
    def export(self, target: ExportTarget, out_path: str) -> None: ...
```
